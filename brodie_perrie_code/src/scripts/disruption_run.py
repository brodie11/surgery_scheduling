...
period_start_year = 2015 #can go 2015-03-01 earliest
...
output_db_location_to_use = OUTPUT_DB_DIR_TEST if testing == True else OUTPUT_DB_DIR 
        ...
        for allowed_overtime in overtimes_considered:
            ...
            for is_disruption_considered in is_disruption_considered_options:
                ...
                for week in range(1, weeks + 1):
                    ...
                    perfect_info_string = "True" if perfect_info_bool else "False"
                    ...
                    suffix_for_iter = f"i_{iter}_pi_{'T' if perfect_info_string == 'True' else 'F'}_pc_{percentile_value}_ao_{allowed_overtime}_idc_{is_disruption_considered_string}_" + suffix
                    ...
                    with Session() as session:
                        ...
                        if inconvenience_sol is None or solve_anyway:
                            ...
                            if week == 1:
                                print(f"iter{iter}")
                                schedule = inconvenienceProb(sim_start_date=simulation_start_date, iter=iter, surgeries=waitlist, sessions=all_sess, turn_around=turn_around, obj_type=obj_type, 
                                        is_disruption_considered=is_disruption_considered, max_disruption_parameter=max_disruption_parameter, 
                                        max_disruption_shift=max_disruption_shift, init_assign=week_1_solution, 
                                ...
                                schedule = inconvenienceProb(sim_start_date=simulation_start_date, iter=iter, surgeries=waitlist, sessions=all_sess, turn_around=turn_around, obj_type=obj_type, 
                                                            is_disruption_considered=is_disruption_considered, max_disruption_parameter=max_disruption_parameter, 
                                                            max_disruption_shift=max_disruption_shift, init_assign=last_week_solution, 
                                                            perfect_information=perfect_info_bool, time_lim=time_lim_other_weeks, 
                                                            new_sessions=new_sessions, optimality_gap=optimality_gap, seed=seed)
                ...
                if iter == 1 and perfect_info_bool == True:
                    ...
                    priority_and_warning_times_df_csv = priority_and_warning_times_df_current_iter
                else:
